<!-- CHARACTER HEADER -->
<?php echo $lang['character_header'] ?>

<!-- CHARACTER TABLE -->
<div style="overflow-y: auto;">
    <table>
        <thead>
            <tr>
                <th><?php echo $lang['character'] ?></th>
                <th><?php echo $lang['health'] ?></th>
                <th><?php echo $lang['armor'] ?></th>
                <th><?php echo $lang['energy'] ?></th>
                <th><?php echo $lang['crit_chance'] ?></th>
                <th><?php echo $lang['melee_damage'] ?></th>
                <th><?php echo $lang['starter_weapon'] ?></th>
                <th><?php echo $lang['price'] ?></th>
                <th><?php echo $lang['bonus_buff'] ?></th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    <img src="img/characters/Knight_icon.png" class="CharacterSize" alt="Knight"><br>
                    <a><strong><?php echo $lang['knight'] ?></strong></a>
                </td>
                <td>6(7)</td>
                <td>5(6)</td>
                <td>180(200)</td>
                <td>5</td>
                <td>4</td>
                <td><?php echo $lang['knight_weapon'] ?></td>
                <td><?php echo $lang['knight_price'] ?></td>
                <td class="wrap"><?php echo $lang['knight_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Rogue_icon.png" class="CharacterSize" alt="Rogue"><br>
                    <a><strong><?php echo $lang['rogue'] ?></strong></a>
                </td>
                <td>5(6)</td>
                <td>3(4)</td>
                <td>180(200)</td>
                <td>10</td>
                <td>5</td>
                <td><?php echo $lang['rogue_weapon'] ?></td>
                <td><?php echo $lang['rogue_price'] ?></td>
                <td class="wrap"><?php echo $lang['rogue_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Wizard_icon.png" class="CharacterSize" alt="Wizard"><br>
                    <a><strong><?php echo $lang['wizard'] ?></strong></a>
                </td>
                <td>3(4)</td>
                <td>5(6)</td>
                <td>240(260)</td>
                <td>0</td>
                <td>3</td>
                <td><?php echo $lang['wizard_weapon'] ?></td>
                <td><?php echo $lang['wizard_price'] ?></td>
                <td class="wrap"><?php echo $lang['wizard_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Assassin_icon.png" class="CharacterSize" alt="Assasin"><br>
                    <a><strong><?php echo $lang['assassin'] ?></strong></a>
                </td>
                <td>4(5)</td>
                <td>4(5)</td>
                <td>180(200)</td>
                <td>10</td>
                <td>5</td>
                <td><?php echo $lang['assassin_weapon'] ?></td>
                <td><?php echo $lang['assassin_price'] ?></td>
                <td class="wrap"><?php echo $lang['assassin_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Alchemist_icon.png" class="CharacterSize" alt="Alchemist"><br>
                    <a><strong><?php echo $lang['alchemist'] ?></strong></a>
                </td>
                <td>5(6)</td>
                <td>5(6)</td>
                <td>180(200)</td>
                <td>5</td>
                <td>4</td>
                <td><?php echo $lang['alchemist_weapon'] ?></td>
                <td><?php echo $lang['alchemist_price'] ?></td>
                <td class="wrap"><?php echo $lang['alchemist_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Engineer_icon.png" class="CharacterSize" alt="Engineer"><br>
                    <a><strong><?php echo $lang['engineer'] ?></strong></a>
                </td>
                <td>5(6)</td>
                <td>5(6)</td>
                <td>180(200)</td>
                <td>5</td>
                <td>3</td>
                <td><?php echo $lang['engineer_weapon'] ?></td>
                <td><?php echo $lang['engineer_price'] ?></td>
                <td class="wrap"><?php echo $lang['engineer_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Vampire_icon.png" class="CharacterSize" alt="Vampire"><br>
                    <a><strong><?php echo $lang['vampire'] ?></strong></a>
                </td>
                <td>3(4)</td>
                <td>4(5)</td>
                <td>120(140)</td>
                <td>5</td>
                <td>3</td>
                <td><?php echo $lang['vampire_weapon'] ?></td>
                <td><?php echo $lang['vampire_price'] ?></td>
                <td class="wrap"><?php echo $lang['vampire_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Paladin_icon.png" class="CharacterSize" alt="Paladin"><br>
                    <a><strong><?php echo $lang['paladin'] ?></strong></a>
                </td>
                <td>1(2)</td>
                <td>8(9)</td>
                <td>120(140)</td>
                <td>0</td>
                <td>4</td>
                <td><?php echo $lang['paladin_weapon'] ?></td>
                <td><?php echo $lang['paladin_price'] ?></td>
                <td class="wrap"><?php echo $lang['paladin_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Elf_icon.png" class="CharacterSize" alt="Elf"><br>
                    <a><strong><?php echo $lang['elf'] ?></strong></a>
                </td>
                <td>5(6)</td>
                <td>4(5)</td>
                <td>180(200)</td>
                <td>5</td>
                <td>4</td>
                <td><?php echo $lang['elf_weapon'] ?></td>
                <td><?php echo $lang['elf_price'] ?></td>
                <td class="wrap"><?php echo $lang['elf_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Werewolf_icon.png" class="CharacterSize" alt="Werewolf"><br>
                    <a><strong><?php echo $lang['werewolf'] ?></strong></a>
                </td>
                <td>11(12)</td>
                <td>1(2)</td>
                <td>120(140)</td>
                <td>5</td>
                <td>5</td>
                <td><?php echo $lang['werewolf_weapon'] ?></td>
                <td><?php echo $lang['werewolf_price'] ?></td>
                <td class="wrap"><?php echo $lang['werewolf_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Priest_icon.png" class="CharacterSize" alt="Priest"><br>
                    <a><strong><?php echo $lang['priest'] ?></strong></a>
                </td>
                <td>3(4)</td>
                <td>5(6)</td>
                <td>200(220)</td>
                <td>0</td>
                <td>3</td>
                <td><?php echo $lang['priest_weapon'] ?></td>
                <td><?php echo $lang['priest_price'] ?></td>
                <td class="wrap"><?php echo $lang['priest_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Druid_icon.png" class="CharacterSize" alt="Druid"><br>
                    <a><strong><?php echo $lang['druid'] ?></strong></a>
                </td>
                <td>6(7)</td>
                <td>3(4)</td>
                <td>130(150)</td>
                <td>5</td>
                <td>5</td>
                <td><?php echo $lang['druid_weapon'] ?></td>
                <td><?php echo $lang['druid_price'] ?></td>
                <td class="wrap"><?php echo $lang['druid_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Robot_icon.png" class="CharacterSize" alt="Robot"><br>
                    <a><strong><?php echo $lang['robot'] ?></strong></a>
                </td>
                <td>3(4)</td>
                <td>6(7)</td>
                <td>180(200)</td>
                <td>0</td>
                <td>3</td>
                <td><?php echo $lang['robot_weapon'] ?></td>
                <td><?php echo $lang['robot_price'] ?></td>
                <td class="wrap"><?php echo $lang['robot_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Berserker_icon.png" class="CharacterSize" alt="Berserker"><br>
                    <a><strong><?php echo $lang['berserker'] ?></strong></a>
                </td>
                <td>9(10)</td>
                <td>3(4)</td>
                <td>150(170)</td>
                <td>5</td>
                <td>5</td>
                <td><?php echo $lang['berserker_weapon'] ?></td>
                <td><?php echo $lang['berserker_price'] ?></td>
                <td class="wrap"><?php echo $lang['berserker_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Necromancer_icon.png" class="CharacterSize" alt="Necromancer"><br>
                    <a><strong><?php echo $lang['necromancer'] ?></strong></a>
                </td>
                <td>3(4)</td>
                <td>5(6)</td>
                <td>240(260)</td>
                <td>0</td>
                <td>4</td>
                <td><?php echo $lang['necromancer_weapon'] ?></td>
                <td><?php echo $lang['necromancer_price'] ?></td>
                <td class="wrap"><?php echo $lang['necromancer_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Officer_icon.png" class="CharacterSize" alt="Officer"><br>
                    <a><strong><?php echo $lang['officer'] ?></strong></a>
                </td>
                <td>5(6)</td>
                <td>5(6)</td>
                <td>180(200)</td>
                <td>5</td>
                <td>4</td>
                <td><?php echo $lang['officer_weapon'] ?></td>
                <td><?php echo $lang['officer_price'] ?></td>
                <td class="wrap"><?php echo $lang['officer_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Taoist_icon.png" class="CharacterSize" alt="Taoist"><br>
                    <a><strong><?php echo $lang['taoist'] ?></strong></a>
                </td>
                <td>4(5)</td>
                <td>4(5)</td>
                <td>180(200)</td>
                <td>5</td>
                <td>4</td>
                <td><?php echo $lang['taoist_weapon'] ?></td>
                <td><?php echo $lang['taoist_price'] ?></td>
                <td class="wrap"><?php echo $lang['taoist_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Inter-dimension_Traveler_icon.png" class="CharacterSize" alt="Inter-dimension Traveler"><br>
                    <a><strong><?php echo $lang['interdimension_traveler'] ?></strong></a>
                </td>
                <td>2</td>
                <td>3</td>
                <td>21</td>
                <td>5</td>
                <td>4</td>
                <td><?php echo $lang['interdimension_traveler_weapon'] ?></td>
                <td><?php echo $lang['interdimension_traveler_price'] ?></td>
                <td class="wrap"><?php echo $lang['interdimension_traveler_buff'] ?></td>
            </tr>
            <tr>
                <td>
                    <img src="img/characters/Element_Envoy_icon.png" class="CharacterSize" alt="Element Envoy"><br>
                    <a><strong><?php echo $lang['element_envoy'] ?></strong></a>
                </td>
                <td>4</td>
                <td>4</td>
                <td>320</td>
                <td>5</td>
                <td>4</td>
                <td><?php echo $lang['element_envoy_weapon'] ?></td>
                <td><?php echo $lang['element_envoy_price'] ?></td>
                <td class="wrap"><?php echo $lang['element_envoy_buff'] ?></td>
            </tr>
        </tbody>
    </table>
</div>